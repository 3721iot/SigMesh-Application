...
void ssp_flash_OTP_protect(uint32_t ssp_addr)
{
	ssp_reconfigure(ssp_addr,FLASH_BIT_RATE, FLASH_SSP_CLK, FLASH_DATA_SIZE);
	uint8_t status1 = ssp_flash_read_status_reg(ssp_addr);
	uint8_t status2 = ssp_flash_read_status_reg2(ssp_addr);
	
	ssp_flash_write_enable(ssp_addr);
	ssp_flash_write_status_reg2(ssp_addr,status1,status2|0x38); //Security Registers will be permanently locked;
	flash_poll_busy_bit(ssp_addr);
	ssp_flash_write_disable(ssp_addr);
}

//offset = 0x1000,0x2000,0x3000, otp section size: 512 = 0x200 
uint8_t ssp_flash_OTP_erase(uint32_t ssp_addr,uint32_t offset)
{
	offset &= 0x3000;
    ...
    ssp_put_byte(ssp_addr,FLASH_SEC_REG_ERASE_OPCODE);
...
//offset = 0x1000,0x2000,0x3000, otp section size: 512 = 0x200 
uint8_t ssp_flash_OTP_write(uint32_t ssp_addr,uint32_t offset, uint32_t length, uint8_t *buffer)
{
    unsigned char page_count;
    uint32_t i;
	offset &= 0x3000;
    ...
    if(length > 0)
    {
        ...
        for(i= 0 ;page_count > 0; page_count--)
        {
			...
			//ssp_flash_write_status_reg(ssp_addr,ssp_flash_read_status_reg(ssp_addr) & (~0x9c));
            ssp_flash_write_enable(ssp_addr);
            ssp_put_byte(ssp_addr,FLASH_SEC_REG_PROGRAM_OPCODE);
            ssp_put_byte(ssp_addr,offset >> 16);
            ssp_put_byte(ssp_addr,offset >> 8);
            ssp_put_byte(ssp_addr,offset);
            ssp_enable(ssp_addr);
...
#define FLASH_READ_SINGLE_PACKET_LEN       (SSP_FIFO_SIZE-5)
//offset = 0x1000,0x2000,0x3000, otp section size: 512 = 0x200 
uint8_t ssp_flash_OTP_read(uint32_t ssp_addr,uint32_t offset, uint32_t length, uint8_t *buffer)
{
    uint8_t write_buf[5];
    uint32_t read_times;
    uint8_t last_bytes;
    ...
    for(i=0; i<read_times; i++)
    {
        write_buf[0] = FLASH_SEC_REG_READ_OPCODE;
        write_buf[1] = offset >> 16;
        write_buf[2] = offset >> 8;
        write_buf[3] = offset ;
        write_buf[4] = 0xFF;
