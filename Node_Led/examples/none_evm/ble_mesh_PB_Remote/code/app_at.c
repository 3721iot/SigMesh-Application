...
extern void app_mesh_send_user_adv_packet(uint8_t duration, uint8_t *adv_data, uint8_t adv_len);
extern void app_auto_update_led_state(uint8_t state);
...
static void app_at_recv_cmd_M(uint8_t sub_cmd, uint8_t *data)
{
    ...
    {
        case 'A':
            mesh_proxy_ctrl(ascii_strn2val((const char *)&data[0], 16, 2));
            break;
        case 'B':
...
//            mesh_iv_update(false);
        ...
        case 'H':
            {
                uint8_t adv_data[] = {0x03, 0xff, 0x02, 0xfc};
                app_mesh_send_user_adv_packet(10, adv_data, sizeof(adv_data));
            }         
		 break;
        default:
            break;
